...
public class ReportSavingFragment extends Fragment {
    ...
    private ReportSavingViewModel viewModel;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_report_saving, container, false);
        ...
        RecyclerView rv = view.findViewById(R.id.rvLaporanSaving);
        adapter = new LaporanSavingAdapter();
        rv.setLayoutManager(new LinearLayoutManager(requireContext()));
        rv.setAdapter(adapter);

        viewModel = new ViewModelProvider(this, new ViewModelProvider.AndroidViewModelFactory(requireActivity().getApplication())).get(ReportSavingViewModel.class);
        viewModel.getLaporanSaving().observe(getViewLifecycleOwner(), adapter::setData);
        ...
        if (loginViewModel.isAdmin()) {
            ...
            storeViewModel.getSelectedStoreId().observe(getViewLifecycleOwner(), storeId -> {
                if (storeId != null && storeId > 0) {
                    ...
                    viewModel.loadLaporanSavingByStore(startDate, endDate, storeId);
                } else {
                    // Load all data
                    viewModel.loadLaporanSaving(startDate, endDate);
                }
            });
        } else {
            ...
            loginViewModel.getCurrentUser().observe(getViewLifecycleOwner(), user -> {
                if (user != null && user.getStoreId() != null) {
                    viewModel.loadLaporanSavingByStore(startDate, endDate, user.getStoreId());
                } else {
                    viewModel.loadLaporanSaving(startDate, endDate);
                }
            });
