...
public interface StockInDao {
    ...
    @Query("SELECT SUM(quantity) FROM stock_in WHERE productId = :productId")
    int getTotalStockInForProduct(int productId);
    
    @Query("SELECT * FROM stock_in WHERE storeId = :storeId ORDER BY createdAt DESC")
    List<StockIn> getAllStockInByStore(int storeId);
} 